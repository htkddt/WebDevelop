...
**Consumer repos (no root build):** Start with **[docs/TUTORIAL_BINDINGS.md](docs/TUTORIAL_BINDINGS.md)** for build, FFI bindings, and language-specific paths.
...
## Quick start (this repo = library only)

```bash
make validate
make lib          # minimal: no libmongoc (CI-friendly smoke build)
```

**After each fix, if your host uses chat history / Mongo:** build with Mongo so you compile the same code paths the app loads:

```bash
brew install mongo-c-driver   # once
make lib-mongo                  # same as: make lib USE_MONGOC=1
```

`make validate` intentionally builds **without** Mongo so it passes without the C driver; for persistence, use **`lib-mongo`** when the driver is installed.

There is **no** `ai_bot` binary inside **c-lib**; hosts load or link the library (see **[docs/TUTORIAL_BINDINGS.md](docs/TUTORIAL_BINDINGS.md)**). API reference: [docs/api.md](docs/api.md).
...
c-lib/               # C library only (no executable; can be its own repo)
  ...
  # Consumed as prebuilt .a/.so or .o by consumer repos (inject/link to execute)
c_ai/                # Example C consumer (sibling repo in some layouts)
...
```bash
brew install mongo-c-driver
make lib-mongo    # or: make lib USE_MONGOC=1
```
...
You can compile the engine as a **shared library** and call it from many languages via FFI (see **`docs/TUTORIAL_BINDINGS.md`**).
...
2. **App integration (build → load → `api_chat`)**  
   **[docs/TUTORIAL_BINDINGS.md](docs/TUTORIAL_BINDINGS.md)** — modes, Mongo, HTTP/C consumers, link line, env checklist.

3. **Tutorial and examples**
   **[docs/TUTORIAL_BINDINGS.md](docs/TUTORIAL_BINDINGS.md)** — per-language FFI (C/C++, Python, Java, Node, Go, Rust, etc.), public C API surface, and toolchain links. For **intent-based temperature** (TECH/CHAT/DEFAULT) and the mini AI switch, see **[.cursor/smart_topic_ai_switch.md](.cursor/smart_topic_ai_switch.md)**. Prefer **`api.h`** / **`api_chat`** for new apps ([api.md](docs/api.md)).
...
2. **Upload** the tarball to your artifact server (Artifactory, Nexus, S3, GitHub Releases, or a private Docker image).  
   See **[docs/TUTORIAL_BINDINGS.md](docs/TUTORIAL_BINDINGS.md)** for curl/aws/gh/Docker examples and consumer usage.
...
3. **Free option (no paid registry):** Use **GitHub Actions** to build and publish to **GitHub Releases**.  
   ...
   Details: [docs/TUTORIAL_BINDINGS.md § Free option](docs/TUTORIAL_BINDINGS.md#free-option-github-actions--releases-no-paid-registry).
