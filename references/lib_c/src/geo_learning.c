...
static int s_vector_gen_backend; /* 0=custom (hash), 1=ollama — from engine config */
...
static void process_turn(const geo_turn_item_t *item) {
    ...
    if (s_vector_gen_backend == API_VECTOR_GEN_CUSTOM) {
        /* Respect engine config: use built-in hash vector, no Ollama embed */
        memset(&emb_opt, 0, sizeof(emb_opt));
        emb_opt.backend = M4_EMBED_BACKEND_CUSTOM;
    } else {
            ...
            if (embed_ret == 0 && embed_dim > 0) {
                ...
                if (storage_redis_connected(s_storage) &&
                    ...
                    if (insert_ret == 0) {
                        ...
                        /* LLM alias discovery — ask Ollama for alternative names */
                        {
                            char alias_prompt[1024];
                            snprintf(alias_prompt, sizeof(alias_prompt),
                                     "List all known names, aliases, and abbreviations for the place \"%s\""
                                     " (including: official name, common name, historical name, "
                                     "Vietnamese with/without accents, English, abbreviation). "
                                     "Return ONLY a JSON array of strings. No explanation.",
                                     name);
                            char alias_out[2048];
                            alias_out[0] = '\0';
                            int ar = ollama_query(OLLAMA_DEFAULT_HOST, OLLAMA_DEFAULT_PORT, NULL,
                                                  alias_prompt, alias_out, sizeof(alias_out));
                            if (ar == 0 && alias_out[0]) {
                                /* Parse JSON array: ["name1","name2",...] */
                                const char *p = strchr(alias_out, '[');
                                if (p) {
                                    p++;
                                    while (*p) {
                                        while (*p == ' ' || *p == '\n' || *p == ',') p++;
                                        ...
                                        if (*p == '"') {
                                            ...
                                            char alias_name[256];
                                            size_t ai = 0;
                                            while (*p && *p != '"' && ai < sizeof(alias_name) - 1) {
                                                if (*p == '\\' && p[1]) p++;
                                                alias_name[ai++] = *p++;
                                            }
                                            alias_name[ai] = '\0';
                                            ...
                                            if (alias_name[0]) {
                                                ...
                                                if (syn) m4_synonym_add(syn, alias_name, name);
                                            }
                                        } else {
                                            break;
                                        }
                                ...
                                fprintf(stderr, "[GEO_LEARNING] alias discovery for '%s': %s\n", name,
                                        alias_out[0] ? "parsed" : "empty");
                            }
                        }
                    } else {
...
int geo_learning_init(struct storage_ctx *st, int vector_gen_backend) {
    if (!st) return -1;
    s_storage = st;
    s_vector_gen_backend = vector_gen_backend;
