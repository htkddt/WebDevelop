...
/* Ollama's last NDJSON line is often {"response":"","done":true,"context":[...]} — no text token. */
static int ol_stream_line_done_true(const char *line) {
    const char *d = strstr(line, "\"done\"");
    if (!d)
        return 0;
    d += 6;
    while (*d == ' ' || *d == '\t')
...
static void ol_stream_flush_line(ol_stream_wrdata_t *st) {
    ...
    if (extract_stream_token_json(st->pending, frag, sizeof(frag)) != 0) {
        if (ol_debug_chat_on() && st->pending_len > 0 && !ol_stream_line_done_true(st->pending)) {
            size_t n = st->pending_len < 280 ? st->pending_len : 280;
            fprintf(stderr,
