...
#define MODEL_SWITCH_URL_MAX     512
#define MODEL_SWITCH_KEY_MAX     256
 ...
 */
/**
 * One row of app config: lane key → model + optional endpoint override.
 *
 * When api_url is set: ai_agent calls that exact endpoint with api_key (direct pin, no pool).
 * When api_url is NULL: ai_agent uses the default cloud pool routing (rate-limit aware).
 * model NULL/empty: uses env M4_MODEL_<KEY>, then OLLAMA_MODEL, then OLLAMA_DEFAULT_MODEL.
 */
typedef struct model_switch_lane_entry {
    const char *key;
    const char *model;      /* model id to send (e.g. "llama-3.1-70b", "qwen2.5:7b") */
    const char *inject;     /* optional system inject text */
    const char *api_url;    /* NULL = use default pool; non-NULL = direct endpoint (e.g. "https://api.groq.com/openai/v1/chat/completions") */
    const char *api_key;    /* NULL = use default env keys; non-NULL = key for this lane's endpoint */
...
typedef struct model_switch_profile {
    ...
    /** When set: direct endpoint for this lane (bypasses cloud pool). Empty = use default pool routing. */
    char api_url[MODEL_SWITCH_URL_MAX];
    char api_key[MODEL_SWITCH_KEY_MAX];
