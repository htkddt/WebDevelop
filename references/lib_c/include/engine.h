...
#define ENGINE_VERSION "1.0.0-beta"
...
typedef struct engine_config {
    ...
    /** Optional: SharedCollection cluster / registry (mirrors api_options_t; NULL/"" = unset). */
...
/** Append one turn (input + assistant) as single document — new shape per .cursor/mongo.md §0. vector/lang/score from Phase 1 when set. embed_model_id: model that generated vector (NULL → ""). llm_model_id: completion route (NULL → omit). */
