...
| 3 | `engine_init(engine)` — `storage_connect()`; if `config.smart_topic_opts` is set and enabled, calls **initial_smart_topic()** (see [.cursor/smart_topic_ai_switch.md](../.cursor/smart_topic_ai_switch.md)). |
...
Ollama is invoked from a worker thread; the UI shows “thinking” until the thread signals done. See [.cursor/smart_topic_ai_switch.md](../.cursor/smart_topic_ai_switch.md).
...
|------|-----|-------------|------------------------------|
| **A — No stream** | `api_chat(ctx, ..., NULL, NULL)` | Cloud pool (Groq → Cerebras → Gemini) or Ollama fallback | Full response in buffer |
| **B — Stream** | `api_chat(ctx, ..., stream_cb, ud)` | `ollama_query_stream` → `POST /api/generate` with `"stream":true` | Each NDJSON line → `extract_stream_token_json()`; fragments concatenated into `full` (UTF-8 safe) |
...
### 9.2 Case 2 — With stream (`api_chat` with `stream_cb != NULL`)
...
| **Smart topic** | `config.smart_topic_opts` / `opts.smart_topic_opts` | Init at `engine_init`; micro-query + temperature in chat/query path. See [.cursor/smart_topic_ai_switch.md](../.cursor/smart_topic_ai_switch.md). |
...
| **Input → extracted text** | `api_chat` (sync or stream) | Non-stream: one-body `extract_generate_reply_body`. Stream: per-line `extract_stream_token_json` → `full` (UTF-8 safe accumulator). See **§9 SEQUENCE INPUT DATA**. |
...
For public API details, see `.cursor/rule.md`, [.cursor/smart_topic_ai_switch.md](../.cursor/smart_topic_ai_switch.md), and `include/*.h`.
